...
def create_app() -> FastAPI:
    ...
    async def ensure_defaults():
        async with SessionLocal() as db:
            if settings.init_default_kb_on_startup:
                result = await db.execute(
                    select(KnowledgeBase).where(KnowledgeBase.name == "default").order_by(KnowledgeBase.created_at.asc())
                )
            ...
            if settings.init_admin_on_startup:
